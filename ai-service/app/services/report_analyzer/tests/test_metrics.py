...
Cobre validação de metric_type (AT-M4) e persistência de record_ultra_batch_complete (AT-M3).
"""
import pytest
...
class TestRecordMetricCall:
    ...
    def test_rejects_invalid_metric_type(self, mock_get_firestore):
    ...
    def test_accepts_automatica_and_persists(
        self, mock_date, mock_get_firestore, mock_update_daily
    ):
    ...
    @patch("app.services.metrics._update_daily_total")
    @patch("app.services.metrics.get_firestore_client")
    def test_accepts_personalized_and_persists(self, mock_get_firestore, mock_update_daily):
...
class TestRecordUltraBatchComplete:
    ...
    @patch("app.services.metrics.get_firestore_client")
    def test_calls_firestore_and_updates_document(self, mock_get_firestore):
